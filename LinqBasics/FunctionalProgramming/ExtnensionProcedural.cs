namespace Linq.FunctionalProgramming
{
    public static class ExtnensionProcedural
    {
        // we want to update this by using functional programming
        // we must make this method extention on IEnumerable<Employee> source so we can filtter over the list dirctly
        public static IEnumerable<Employee> GetEmployeesWithFirstNameStartsWith(string value)
        {   
            //we must send repo data to the method
            var employees = Repository.LoadEmployees();
            // we must send Func to filter data to make 1 method not to many
